...
def chessDrawer(initialState: Array[Array[String]]): Unit = {
  ...
  def createGamePanel(): JPanel = {
    new JPanel(new GridLayout(8, 8)) {
      for {
        ...
        val foreColor: Color = if (row <= 1) Color.black else Color.white
        if (isSame(row, col)) {
          add(createCellLabel(new Color(225, 210, 156), foreColor, initialState(row)(col)))
        } else {
          add(createCellLabel(new Color(119, 149, 86), foreColor, initialState(row)(col)))
        }
      }
